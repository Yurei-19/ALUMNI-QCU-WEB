...
try {
    if ($add_id !== null) {
        ...
        $stmt = $conn->prepare("INSERT INTO add_alumni_list SELECT * FROM reject_alumni_list WHERE add_id = :add_id");
        $stmt->bindParam(':add_id', $add_id);
        $stmt->execute();

        // Delete data from source_table after transfer
        $deleteStmt = $conn->prepare("DELETE FROM reject_alumni_list WHERE add_id = :add_id");
